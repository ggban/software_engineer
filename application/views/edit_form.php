...
<body bgcolor="#E6E6FA">
<div class="container">
	<form role="form" id="form_index" class="form-horizontal" action=<?php echo base_url("formController/form_update");?>>
...
$(document).ready(function(){
	...
	var form_data=<?php  if(isset($form_data))print_r($form_data);else echo "''"?>;

	
	if(form_data!='')
	{	
		$(form_title).val(form_data.title);
		$(form_remark).val(form_data.remark);
		...
		{ 
	       ...
	       if (isNumber(key)) 
	       {
	       		...
	       		add_question(obj);
	       		question_type($( "#question_area #question:last #type-val")[0]);

		        for (var prop in obj) {
		          // important check that this is objects own property 
		          // not from prototype prop inherited
		          if(isNumber(prop)){
		          	single_add($( "#question_area #question:last .btndiv")[0],obj[prop])
	...
	*/
	...
	function single_add(dom,data){
		if (typeof data === 'undefined') { data = ''; }
		$(dom).before('<div class="row form-group qrow"><div id="sq" class="sque col-sm-5 col-sm-offset-1"><input type="text" class=" choices" value="'+data+'">&nbsp;<a id="minus_btn" class="btn btn-danger btn-xs" onclick="remove_btn(this)"><span class="glyphicon glyphicon-minus"></span></a></div></div>');
	}
	...
	function add_question(data)
	{
		...
		if(data!='')
		{
			//console.log($( "#question_area #question:last #question-remark" ));
			$( "#question_area #question:last #question-input" ).val(data.question);
			$( "#question_area #question:last #remark-input" ).val(data.qremark);
			$( "#question_area #question:last #type-val" ).val(data.type);
			$( "#question_area #question:last #required").prop("checked",(data.required==="true"));
